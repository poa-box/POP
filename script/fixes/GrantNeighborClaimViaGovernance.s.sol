...
/*
 * ============================================================================
 * Grant the Neighbor hat CLAIM permission via HybridVoting governance
 * ============================================================================
 *
 * Target org: Decentral Park (Gnosis).
 *
 * Today the "Neighbor" (member) hat has a global TaskManager mask of 0x00 — it
 * holds no task permissions, so members can only be *assigned* tasks top-down;
 * they cannot self-claim an open task or apply to an application-gated one
 * (both claimTask and applyForTask require the CLAIM bit). This proposal adds
 * the CLAIM bit (1 << 1 = 0x02) to the Neighbor hat so members can self-serve.
 *
 * Read-modify-write: setConfig(ROLE_PERM, ...) REPLACES the mask (it does not
 * OR-merge). We read the current Neighbor mask and OR in CLAIM so any bits set
 * between now and execution are preserved rather than clobbered.
 *
 * Single-call batch (executed by the Executor on pass):
 *   1. setConfig(ROLE_PERM, abi.encode(NEIGHBOR_HAT, currentMask | CLAIM))
 *
 ...
 * onlyCreator + class-hat checks) and asserts the post-state mask has CLAIM set
 * and equals (currentMask | CLAIM).
 *
 * Usage:
 *   # Sim (no broadcast — validate end-to-end on a fork)
 *   FOUNDRY_PROFILE=production forge script \
 *     script/fixes/GrantNeighborClaimViaGovernance.s.sol:SimGrantNeighborClaimDecentralPark \
 *     --fork-url gnosis -vvv
 *
 *   # Broadcast (creates the real proposal; members vote in normal cadence)
 *   source .env && FOUNDRY_PROFILE=production forge script \
 *     script/fixes/GrantNeighborClaimViaGovernance.s.sol:BroadcastGrantNeighborClaimDecentralPark \
 *     --rpc-url gnosis --broadcast --slow
 *
 * Optional env overrides:
 *   GRANT_HAT_ID    — override the target hat (default = Decentral Park Neighbor)
...
// Neighbor is the member hat; it is already a HybridVoting creator hat, so any
// Neighbor/Delegate wearer can broadcast this proposal.
...
/// Configure via vm.store(addr, slot=0, godmode).
...
abstract contract GrantNeighborClaimBase is Script {
    ...
    /// @dev Build the single-call batch: set the hat's global mask to (current | CLAIM).
        ...
        returns (IExecutor.Call[] memory batch, uint8 currentMask, uint8 newMask)
    {
        ...
        newMask = currentMask | TaskPerm.CLAIM;
    ...
    function _printPreview(uint256 hatId, uint8 currentMask, uint8 newMask) internal pure {
        console.log("  Target hat:        ", hatId);
        console.log("  Current ROLE_PERM: ", currentMask);
        console.log("  New ROLE_PERM:     ", newMask, "(adds CLAIM = 0x02)");
    }
    ...
    function _simFullFlow(string memory orgName, address taskManager, address hybridVoting, uint256 targetHat)
        internal
    {
        console.log("\n=== Neighbor CLAIM grant sim:", orgName, "===");
        ...
        address voter = makeAddr("neighbor-claim-sim-voter");
        ...
        require(newMask != currentMask, "Sim: proposal is a no-op (CLAIM already set)");
        _printPreview(targetHat, currentMask, newMask);

        IExecutor.Call[][] memory batches = new IExecutor.Call[][](1);
        batches[0] = batch;

        // 3. Create the proposal (MIN_DURATION = 10 minutes for sim speed).
        uint32 minutesDuration = 10;
        uint256[] memory pollHats = new uint256[](0); // unrestricted poll
        vm.prank(voter);
        HybridVoting(hybridVoting)
            .createProposal(
                bytes(string.concat("Grant Neighbor CLAIM - ", orgName)),
                bytes32(0),
                minutesDuration,
                1,
                batches,
        ...
        // 7. Verify post-state: CLAIM bit set, and mask equals (current | CLAIM).
        uint8 maskAfter = _readRolePermGlobal(taskManager, targetHat);
        require(maskAfter == newMask, "Sim: post-state mask mismatch");
        require(maskAfter & TaskPerm.CLAIM != 0, "Sim: CLAIM bit not set");
        console.log("  Post-state mask:", maskAfter, "(CLAIM set, prior bits preserved)");
        console.log("PASS:", orgName, "Neighbor CLAIM governance grant executed end-to-end.");
    }
        ...
        internal
    {
        ...
        console.log("\n=== Broadcasting Neighbor CLAIM grant proposal:", orgName, "===");
        ...
        (IExecutor.Call[] memory batch, uint8 currentMask, uint8 newMask) = _buildBatch(taskManager, targetHat);
        require(newMask != currentMask, "Broadcast: proposal is a no-op (CLAIM already set on target hat)");
        _printPreview(targetHat, currentMask, newMask);
...
contract SimGrantNeighborClaimDecentralPark is GrantNeighborClaimBase {
    function run() public {
        _simFullFlow(
            "Decentral Park", DECENTRAL_PARK_TM, DECENTRAL_PARK_HV, _resolveTargetHat(DECENTRAL_PARK_NEIGHBOR_HAT)
        );
    }
}

contract BroadcastGrantNeighborClaimDecentralPark is GrantNeighborClaimBase {
    function run() public {
        _broadcastProposal(
            "Decentral Park", DECENTRAL_PARK_TM, DECENTRAL_PARK_HV, _resolveTargetHat(DECENTRAL_PARK_NEIGHBOR_HAT)
        );
