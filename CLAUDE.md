...
- Do NOT use OpenZeppelin `AccessControl` — role-based access uses the per-org `MembershipAuthority` (Access v2) on the authority path, with a legacy Hats Protocol arm surviving in the dual-path modules ONLY until the last live org migrates (Wave G strips it)
...
- LEGACY (until Wave G): the hat-tree/`HatManager` arm in the dual-path modules serves
  unmigrated live orgs only — do NOT build new features on it
...
- Check permissions via `MembershipAuthority.hasPerm()/isMember()` on the authority path (legacy arm: `HatManager.hasAnyHat()`, never raw `hats.isWearerOfHat()` alone)
