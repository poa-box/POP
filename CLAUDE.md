...
- Do NOT use OpenZeppelin `AccessControl` — role-based access uses the per-org `MembershipAuthority` (Access v2). Wave G removes legacy Hats authorization; unmigrated orgs are inactive
...
**Access control — Access v2 (`MembershipAuthority`):**
- ONE authority per org (BeaconProxy, ERC-7201): subjects = roles AND groups (composition, not
  tree ancestry); membership = accepted × eligible (tri-state fold: explicit rule > attestor
  (vouch/zk-email) > subject default; bans are supreme); semantic perm table
  `perm[subject][key][ctx]` (`AccessV2PermKeys`; TM per-project ctx = projectId + 1 per W4);
  delegation via manager configs + pending actions with review delays; sticky
  (delegable=false) governance rules are untouchable by delegates
- `AuthorityRouter` (protocol singleton, CREATE3-identical both chains) is the chain-wide read
  path: PaymasterHub + OrgRegistry point at it; adopted legacy hat ids bind per org, new-style
  ids (< 2^224, authority-embedded) self-route; unbound legacy ids pass through to real Hats
- New orgs deploy authority-native (no Hats tree / EligibilityModule / ToggleModule); the four
  pre-v2 orgs migrate via `script/accessv2/MIGRATION-RUNBOOK.md`
- Wave G: per-org modules require MembershipAuthority. Historical Hats/config slots and proposal
  structures stay intact for upgrade safety, but never authorize operations. AuthorityRouter remains
  permanent ID-continuity infrastructure; keep historical subgraph templates and start blocks
...
- Check permissions via `MembershipAuthority.hasPerm()/isMember()`; never restore a legacy Hats fallback
